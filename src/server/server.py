...
RELEASE_VERSION = "4.4.1-dev.93" # Public release version code
...
def check_blank_admin_creds_warning():
    '''Sets/clears the admin-page warning for blank admin credentials.
    Blank credentials only grant open access when Server Debug Mode is
    enabled; otherwise every guarded HTTP route and SocketIO event will
    reject all requests. Also logs to the server console, since once
    that lockout takes effect the admin pages that would show this
    warning are themselves unreachable.
    '''
    username = RaceContext.serverconfig.get_item('SECRETS', 'ADMIN_USERNAME')
    password = RaceContext.serverconfig.get_item('SECRETS', 'ADMIN_PASSWORD')
    debug_mode = RaceContext.serverconfig.get_item('GENERAL', 'DEBUG')
    if not username and not password and not debug_mode:
        message = __('Admin username/password are blank, which will result in all admin pages'
                     ' and actions being blocked; need to set a username and password')
        RaceContext.rhui.set_ui_message('blank-admin-creds', message,
            header='Warning', subclass='blank-admin-creds')
        logger.warning(message)
    else:
        RaceContext.rhui.clear_ui_message('blank-admin-creds')
    SOCKET_IO.emit('update_server_messages', RaceContext.rhui.get_ui_server_messages_str())
...
def apply_default_admin_creds_if_blank():
    ...
    if not RaceContext.serverconfig.get_item('SECRETS', 'ADMIN_USERNAME') and \
        ...
        RaceContext.rhui.emit_priority_message(
            __('Blank admin credentials are not allowed; '
...
def on_set_config(data):
    ...
    if data['section'] in ('SECRETS', 'GENERAL'):
        ...
        check_blank_admin_creds_warning()
    Events.trigger(Evt.CONFIG_SET, {
